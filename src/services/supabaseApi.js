...
const invalidateCache = (type) => {
  ...
  if (type.includes('expense')) {
    ...
    emit(DataEvents.BUDGETS_CHANGED);
  } else if (type.includes('goal')) {
    emit(DataEvents.GOALS_CHANGED);
  } else if (type.includes('recurring')) {
    emit(DataEvents.RECURRING_CHANGED);
  } else if (type.includes('scheduled')) {
    emit(DataEvents.SCHEDULED_CHANGED);
  } else if (type.includes('rule') || type.includes('autoRule')) {
